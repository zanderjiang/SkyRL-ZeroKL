set -x
# Minimal OOM-SAFE diagnostic: 1 GPU, 1 engine, local model + data, few steps.
# Goal: capture [ZEROKL-MISS] (weight-sync name mismatch) + [ZEROKL-SYNC] + RAM curve
# WITHOUT the 8-engine footprint that OOM'd the headnode. Console logger, no wandb.
...
export SKYRL_ZEROKL_MAX_MODEL_LEN=1024
...
uv run --isolated --extra megatron -m examples.train.algorithms.dapo.main_dapo \
  ...
  trainer.run_name="zerokl_diag_1gpu_nochunk" \
